/**
 * Metadata Extractor
 * Extracts product title, image, and URL metadata with comprehensive logging
 */
...
export async function extractTitle(page) {
  ...
  console.log(`✅ Title extraction completed in ${extractionTime}ms: ${title || 'Not found'}`);
...
export async function extractImage(page) {
  ...
  const extractionTime = Date.now() - extractionStart;
  console.log(`✅ Image extraction completed in ${extractionTime}ms: ${image ? 'Found' : 'Not found'}`);
...
export async function extractCanonicalUrl(page) {
  ...
  const extractionTime = Date.now() - extractionStart;
  console.log(`✅ Canonical URL extraction completed in ${extractionTime}ms: ${url}`);
...
export function extractVendor(url) {
  console.log(`🔍 Extracting vendor from URL: ${url}`);
  ...
  try {
    ...
    const vendor = vendorName.charAt(0).toUpperCase() + vendorName.slice(1);
    
    console.log(`✅ Extracted vendor: ${vendor}`);
    return vendor;
  } catch (error) {
    console.warn(`⚠️ Failed to extract vendor from URL: ${error.message}`);
    return 'Unknown';
  }
}
...
export async function extractVariants(page) {
  ...
  const extractionTime = Date.now() - extractionStart;
  console.log(`✅ Variants extraction completed in ${extractionTime}ms`);
  console.log(`   🏷️  Brand: ${variants.brand || 'Not found'}`);
  console.log(`   🎨 Color: ${variants.color || 'Not found'}`);
  console.log(`   💾 Capacity: ${variants.capacity || 'Not found'}`);
  console.log(`   📏 Size: ${variants.size || 'Not found'}`);
...
export async function extractMetadata(page) {
  ...
  console.log(`   🎨 Color: ${metadata.color || 'Not found'}`);
  console.log(`   💾 Capacity: ${metadata.capacity || 'Not found'}`);
  console.log(`   📏 Size: ${metadata.size || 'Not found'}`);
  console.log(`   🖼️  Image: ${metadata.image ? 'Found' : 'Not found'}`);
  console.log(`   🌐 URL: ${metadata.url}`);
  console.log(`   ⏱️  Total extraction time: ${extractionTime}ms`);
